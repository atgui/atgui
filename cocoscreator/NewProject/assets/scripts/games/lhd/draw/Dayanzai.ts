...
export default class Dayanzai extends Draw {
    private _MAX_LINE = 6;
    ...
    public async _drawDayanzai(daluArray: Array<Array<LhdRecordModel>>) {
        ...
        if (daluArray.length < 2) {
            return;
        }
        if (daluArray.length == 2 && daluArray[1].length < 2) {
            return;
        }
        //取最后一个
        ...
        if (arrays.length == 1) {//是第一行,看前面两列长度是否一样
            ...
            let len2 = daluArray[daluArray.length - 3].length;
            if (len1 == len2) {
                mod.type = 0;
            } else {
                mod.type = 2;
            ...
            if (daluArray[daluArray.length - 2][arrays.length - 1]) {
                //红
                mod.type = 0;
            } else {
                mod.type = 2;
                //前面一个没有,判断当前列比上一列多出一个后就画红
                let len1 = daluArray[daluArray.length - 2].length;// ? daluArray[daluArray.length - 3].length : 0;
