import { ISocket, CallbackObject, NetData, INetworkTips, IProtocolHelper, RequestObject, NetCallFunc } from "./INetInterface";
...
export class SocketNode {
    ...
    public send(buf: NetData, force: boolean = false): boolean {
        ...
        } else if (this.state == NetNodeState.Checking ||
            this.state == NetNodeState.Connecting) {
            this._requests.push({
                buffer: buf,
                rspCmd: 0,
